...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Login successful, returns JWT token"),
...
pub async fn login(
    ...
    let mut conn = pool
        .get()
        .expect("There was a problem getting the database pool.");
    ...
    let secret =
        std::env::var("JWT_SECRET_KEY").expect("There's not JWT_SECRET_KEY, please define it.");
    let token = encode(
        &Header::default(),
        &claims,
        &EncodingKey::from_secret(secret.as_bytes()),
    )
    .expect("Error encoding the JWT token");
...
pub async fn register(
    ...
    if existing_user.is_some() {
        return (
            StatusCode::CONFLICT,
            Json(ApiResponse::error("User already exists.")),
        );
    }
